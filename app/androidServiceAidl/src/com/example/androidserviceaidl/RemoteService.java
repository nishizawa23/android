...
public class RemoteService extends Service {
	...
	/**
	 * TAG,用于输出Log，在这里，我们把pid，即进程的ID也记录上 这样就可以比较Service的进程和Client的进程是否在同一进程
	 */
	private static final String TAG = "RemoteService, PID=" + Process.myPid();

	/**
	 * 定义RemoteService的Binder，在这个Binder里面，我们实现了一个
	 * 函数，getCount()，用于计算getCount()函数被调用的次数。 这个函数需要到IRemoteService.aidl注册。
	 */
	...
	private final IRemoteService.Stub mBinder = new IRemoteService.Stub() {
		...
		@Override
		public int getPid(ITaskServiceCallBack callback) throws RemoteException {
